	...
	if (file !== undefined) {
		...
		const model = genAI.getGenerativeModel({
			...
			systemInstruction:
				...
				"For every every problem, give a chain of thought and before solving it, write code to solve the problem and use it to guide you. Display all of your math in LATEX format using single and double dollar sign delimiters. For each step of the problem that requires algebra (for example, if you simplify, expand, or substitute, etc), write code to accomplish the task and run it to get your desired result BEFORE moving on. If a portion requires reasoning rather than math, do not write code for that portion. THIS IS VERY IMPORTANT: CHECK ALL OF YOUR CALCULATIONS WITH CODE AGAIN AFTER YOU COMPLETE THE PROBLEM AT ALL TIMES. If your answer does not match the code's answer, THE CODE's ANSWER IS MORE LIKELY TO BE CORRECT, so you should recheck your work according to the code's response unless you are absolutely sure that you are correct. If the question requires an exact answer rather than a decimal one, use your code and result to infer the correct answer. Show your steps briefly. Include your final answer at the very bottom of your response after writing '(final_answer)'",
			tools: [{codeExecution: {}}],
		});
		...
		let generatedContentStream = await model.generateContentStream([prompt, ...imageParts]);
